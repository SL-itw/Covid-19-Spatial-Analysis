...
legend_scale_ses <- data.frame(
  SES = c(rep(1, 3), rep(2, 3), rep(3, 3)),
...
legend <- ggplot() + 
  geom_tile(
    data = legend_scale_ses,
    aes(x = SES, y = case_rate, fill = color)
  ) + 
  scale_fill_identity() +
  labs(x = "SES →",
       y = "Case Rate →") +
...
r <- raster("./Covid-19_NYC/AnnAvg1_10_300mRaster")
